...
function defineTool(definition: Tool): Tool {
  return definition;
}
...
const recordingStart = defineTool({
  ...
  handler: async (args: { projectId: string; runId?: string }, context: any) => {
    const browser = context.browser || context.page?.context()?.browser();
    if (!browser) {
      throw new Error('Browser instance not available');
    }

    const result = await startRecordingSession(browser, args.projectId, args.runId);
    ...
    if (result.success) {
      ...
      return {
        result: `Failed to start recording: ${result.message}`,
        isError: true
      };
    }
  }
...
const recordingStop = defineTool({
  ...
  handler: async (args: any, context: any) => {
    ...
    if (result.success) {
      ...
      return {
        result: `Failed to stop recording: ${result.message}`,
        isError: true
      };
    }
  }
...
const recordingStatus = defineTool({
  capability: 'recording',
  schema: {
...
const recordingList = defineTool({
  capability: 'recording',
  schema: {
  ...
  handler: async (args: { limit?: number }, context: any) => {
    ...
    return {
      result: `Recording Sessions (${result.recordings.length}/${result.total}):\n\n${recordingsList}`
    };
  }
});

const recordingArtifacts = defineTool({
  capability: 'recording',
  schema: {
  ...
  handler: async (args: { projectId: string; runId: string }, context: any) => {
    ...
    if (result.success && result.artifacts) {
      ...
      return {
        result: `Failed to get artifacts: ${result.message}`,
        isError: true
      };
    }
  }
